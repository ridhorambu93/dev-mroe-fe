...
export default function AdminFeedback() {
  ...
  const avgRating = data.length
    ? (data.reduce((sum, r) => sum + (r.rating || 0), 0) / data.length).toFixed(
        1,
      )
    : "0"
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Moderasi rating dan komentar dari pengguna.
        </p>
      </div>
            ...
            <tbody>
              ...
              {!loading &&
                paginated.map((row, i) => (
                  <tr
                    key={row.id}
                    className={`border-b last:border-0 hover:bg-gray-50 ${row.is_hidden ? "opacity-50" : ""}`}>
